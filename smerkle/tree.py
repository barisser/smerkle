...
def DEFAULT_HASH_FUNCTION(x): return hashlib.sha256(
    str(x).encode()).hexdigest()


MAX_DEPTH = 64
# this value can't be inserted into the tree.
KNOWN_RANDOMNESS = "0122145a87e25335b71ee0562fc590e4b3754f1b8feb06a44ecdb22ec996cfa4"
...
empty_hashes = dict()
empty_hashes[MAX_DEPTH] = DEFAULT_HASH_FUNCTION(
    str(MAX_DEPTH) + KNOWN_RANDOMNESS)
...
class SMT:
    def __init__(self, max_depth=MAX_DEPTH, hashfunction=DEFAULT_HASH_FUNCTION, dump=None):
        ...
        self.empty_hashes[max_depth] = self.hash(
            str(max_depth) + KNOWN_RANDOMNESS)
    ...
    def add_to_next_leaf(self, value):
        # technically nodes added at non-leaves make this non-monotonic, but OK.
        self.add_node(self.n_elements, self.max_depth, value)
